#!/bin/bash
# Deploy MTT Registry Worker via Cloudflare REST API
# Termux-compatible (no wrangler)
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WORKER_NAME="2a-mtt-registry"
...
echo "[2/2] Uploading worker '$WORKER_NAME'..."
curl -sS --fail \
...
echo ""
echo "NOTE: Worker runs in stub mode. Bind KV/R2 in Dashboard for full functionality."
