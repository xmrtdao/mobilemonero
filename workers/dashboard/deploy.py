#!/usr/bin/env python3
"""Deploy dashboard Worker via Cloudflare REST API."""
...
def deploy():
    ...
    print(f"Deploying dashboard Worker ({len(js_code)} bytes)...")

    # Upload raw JS
    cmd = [
        "curl", "-s", "-X", "PUT",
        f"https://api.cloudflare.com/client/v4/accounts/{ACCOUNT_ID}/workers/scripts/dashboard",
    ...
    # Ensure route exists
    print("Ensuring route dashboard.mobilemonero.com/* ...")
    route_payload = json.dumps({
        "pattern": "dashboard.mobilemonero.com/*",
        "script": "dashboard",
        "zone_id": ZONE_ID
    })
    cmd2 = [
